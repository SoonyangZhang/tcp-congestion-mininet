...
class NET_EXPORT IOBuffer : public base::SimpleRefCount<IOBuffer> {
 public:
  IOBuffer();
 ...
 protected:
  friend class base::SimpleRefCount<IOBuffer>;
...
class NET_EXPORT DrainableIOBuffer : public IOBuffer {
 public:
  // TODO(eroman): Deprecated. Use the size_t flavor instead. crbug.com/488553
  DrainableIOBuffer(base::scoped_refptr<IOBuffer> base, int size);
  DrainableIOBuffer(base::scoped_refptr<IOBuffer> base, size_t size);
 ...
 private:
  ...
  base::scoped_refptr<IOBuffer> base_;
  int size_;
  int used_;
};
...
class NET_EXPORT GrowableIOBuffer : public IOBuffer {
 ...
 private:
  ...
  std::unique_ptr<char, butil::FreeDeleter> real_data_;
  int capacity_;
  int offset_;
};
