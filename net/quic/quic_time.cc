...
std::string QuicTime::Delta::ToDebuggingValue() const {
  ...
  if (absolute_value > one_s && absolute_value % one_s == 0) {
    return quic::QuicheStringPrintf("%" PRId64 "s", time_offset_ / one_s);
  }
  if (absolute_value > one_ms && absolute_value % one_ms == 0) {
    return quic::QuicheStringPrintf("%" PRId64 "ms", time_offset_ / one_ms);
  }
  return quic::QuicheStringPrintf("%" PRId64 "us", time_offset_);
}
