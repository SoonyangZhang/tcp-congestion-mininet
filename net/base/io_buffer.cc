...
void IOBuffer::AssertValidBufferSize(size_t size) {
  butil::CheckedNumeric<int>(size).ValueOrDie();
}
...
DrainableIOBuffer::DrainableIOBuffer(base::scoped_refptr<IOBuffer> base, int size)
    : IOBuffer(base->data()), base_(std::move(base)), size_(size), used_(0) {
  AssertValidBufferSize(size);
}

DrainableIOBuffer::DrainableIOBuffer(base::scoped_refptr<IOBuffer> base, size_t size)
    : IOBuffer(base->data()), base_(std::move(base)), size_(size), used_(0) {
  AssertValidBufferSize(size);
}
